...
## 5. Стратегия API-мокирования (Mock Server)

Поскольку реального бэкенда для интеграции на текущий момент не существует, для проведения сквозного (E2E) тестирования, демонстрации флоу и "оживления" мобильного клиента внедряется локальный фейковый API (Mock-сервер).

**Техническое решение:** 
Для реализации мок-сервера будет использоваться встроенный `MockEngine` от Ktor Client.
- `MockEngine` будет перехватывать все исходящие HTTP-запросы непосредственно на сетевом слое клиента.
- В ответ на запросы MockEngine будет отдавать заранее заготовленные JSON-ответы.
- Структура ответов, типы данных и статусы (включая цены в копейках) будут строго соответствовать нашим контрактам из `data-schema.md` и спецификации `openapi.yaml`.
- Данный подход позволит полностью протестировать бизнес-логику, обработку ошибок, навигацию и UI без необходимости развертывания реального или внешнего тестового сервера, обеспечивая полную автономность мобильного клиента на этапе QA.
